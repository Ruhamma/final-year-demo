  ...
  return (
    <Box>
      <Box className="px-4 sm:px-10 md:px-24 py-10">
        <Text className="text-lg sm:text-xl md:text-2xl font-semibold">Related Artworks</Text>
        <Text className="text-xs sm:text-sm font-light">{t('Arts by emerging artists')}</Text>
      </Box>
              ...
              <CarouselSlide key={artwork.id}>
                <Link href={`/artworks/${artwork.id}`}>
                  <Card shadow="sm" radius="md" withBorder>
                    ...
                    <Group className="py-2" justify="space-between" align="center">
                      <Text className="text-sm font-semibold">{artwork.title}</Text>
                    </Group>
